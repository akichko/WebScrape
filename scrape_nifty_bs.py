...
class WebScrapeNiftyBs(ws.WebScrapeBs):    
    ...
    def get_record_by_elem(self, element): #element:detail page
        ...
        if len(element.select('.titleMain .name')) > 0:
            record['名前'] = element.select('.titleMain .name')[0].text
        if len(element.select('.otherPrice')) > 0:
            record['管理費等'] = element.select('.otherPrice')[0].text
        return record
...
df_e = controller.start_scraping(controller.make_url('tokyo', 'e', 20000000))
df_e['area'] = 'east'
df_s = controller.start_scraping(controller.make_url('tokyo', 's', 20000000))
df_s['area'] = 'south'
